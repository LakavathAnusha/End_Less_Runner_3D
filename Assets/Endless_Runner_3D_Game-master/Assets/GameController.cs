    ...
    void Update()
    {
        ...
        while(spawnPoint<player.transform.position.x+safeMargin)
        {
           ...
           GameObject currentPlatform = Instantiate(platformPrefab[k], transform.position , Quaternion.identity);
            ...
            currentPlatform.transform.position = new Vector3(spawnPoint + platform .GetComponentInChildren<Transform>().localScale.x/ 2, 0, 0);
            // spawnPoint = 35f;
            spawnPoint = spawnPoint + platform.GetComponentInChildren<Transform>().localScale.x;
        }
    }
}
